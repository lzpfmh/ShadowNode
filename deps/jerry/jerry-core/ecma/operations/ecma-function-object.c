 ...
 */
static inline const ecma_value_t * __attr_always_inline___
                       ...
                       ecma_length_t arguments_list_len) /**< length of arguments list */
{
  ...
  while (true)
  {
...
#ifndef CONFIG_DISABLE_ES2015_CLASS
        if (unlikely (is_class_constructor))
        {
    ...
    {
#ifndef CONFIG_DISABLE_ES2015_CLASS
      if (unlikely (args_len_or_this == ECMA_VALUE_IMPLICIT_CONSTRUCTOR))
      {
                            ...
                            ecma_length_t arguments_list_len) /**< length of arguments list */
{
  ...
  while (unlikely (ecma_get_object_type (func_obj_p) == ECMA_OBJECT_TYPE_BOUND_FUNCTION))
  {
    /* 1-3. */
    ecma_extended_object_t *ext_function_p = (ecma_extended_object_t *) func_obj_p;
...
#ifndef CONFIG_DISABLE_ES2015_ARROW_FUNCTION
  if (unlikely (type == ECMA_OBJECT_TYPE_ARROW_FUNCTION))
  {
    return ecma_raise_type_error (ECMA_ERR_MSG ("Arrow functions have no constructor."));
  }
#endif /* CONFIG_DISABLE_ES2015_ARROW_FUNCTION */

  if (unlikely (type == ECMA_OBJECT_TYPE_FUNCTION && ecma_get_object_is_builtin (func_obj_p)))
  {
  ...
  if (likely (this_arg_value == ECMA_VALUE_UNDEFINED))
  {
    /* 5. */
  ...
  if (unlikely (new_this_obj_p == NULL))
  {
