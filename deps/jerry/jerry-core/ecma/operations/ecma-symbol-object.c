...
/**
 * Definition of the start of a SymbolDescriptiveString
 *
 * See also: 19.4.3.2.1.5
 */
#define ECMA_SYMBOL_STR_START "Symbol("

/**
 * Length of the start of a SymbolDescriptiveString
 *
 * See also: 19.4.3.2.1.5
 */
#define ECMA_SYMBOL_STR_START_LENGTH (sizeof (ECMA_SYMBOL_STR_START) - 1)

/**
 * Definition of the end of a SymbolDescriptiveString
 *
 * See also: 19.4.3.2.1.5
 */
#define ECMA_SYMBOL_STR_END ")"

/**
 * Length of the end of a SymbolDescriptiveString
 *
 * See also: 19.4.3.2.1.5
 */
#define ECMA_SYMBOL_STR_END_LENGTH (sizeof (ECMA_SYMBOL_STR_END) - 1)
...
ecma_prop_name_is_symbol (ecma_string_t *string_p) /**< ecma-string */
{
  ...
  return ECMA_STRING_GET_CONTAINER (string_p) == ECMA_STRING_CONTAINER_HEAP_UTF8_SYMBOL;
...
ecma_get_symbol_descriptive_string (ecma_value_t symbol_value) /**< symbol to stringify */
{
  ...
  /* 5. */
  ecma_string_t *symbol_start_str_p = ecma_new_ecma_string_from_utf8 ((const lit_utf8_byte_t *) ECMA_SYMBOL_STR_START,
                                                                      ECMA_SYMBOL_STR_START_LENGTH);

  ecma_string_t *concat_p = ecma_concat_ecma_strings (symbol_start_str_p, string_desc_p);

  ecma_string_t *final_str_p = ecma_append_chars_to_string (concat_p,
                                                            (const lit_utf8_byte_t *) ECMA_SYMBOL_STR_END,
                                                            ECMA_SYMBOL_STR_END_LENGTH,
                                                            ECMA_SYMBOL_STR_END_LENGTH);
#ifndef JERRY_NDEBUG
  lit_utf8_size_t final_str_length = ecma_string_get_size (final_str_p);
  lit_utf8_size_t string_desc_length = ecma_string_get_size (string_desc_p);
  JERRY_ASSERT (string_desc_length + ECMA_SYMBOL_STR_START_LENGTH + ECMA_SYMBOL_STR_END_LENGTH == final_str_length);
#endif /* !JERRY_NDEBUG */
